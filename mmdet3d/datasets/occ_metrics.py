...
class Metric_mIoU():
    ...
    def add_iou_batch(self, semantics_pred, semantics_gt, mask_lidar, mask_camera):
        self.iou_cnt += 1
    ...
    def count_iou(self):
        ...
        header = ['classes']
        for i in range(2):
            header.append('empty' if i == 0 else 'non-empty')
        header.extend(['miou'])
        ...
        table_columns.append([f'{round(np.nanmean(mIoU[:2 - 1]) * 100, 2):.4f}'])
        table_data = [header]
        ...
        for ind_class in range(2):
            log = '===> {} - IoU = '.format('empty' if ind_class == 0 else 'non-empty')  + str(round(mIoU[ind_class] * 100, 2))
            print_log(log, logger=self.logger)
        return round(mIoU[1] * 100, 2)
