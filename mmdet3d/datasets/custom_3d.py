...
class Custom3DDataset(Dataset):
    ...
    def get_data_info(self, index):
        ...
        if not self.test_mode:
            ...
            if self.filter_empty_gt and ~(annos['gt_labels_3d'] != -1).any():
                return None
        # assert isinstance(input_dict, dict),f"input_dict must be a dict,now is {type(input_dict)}"
        return input_dict
    ...
    def prepare_train_data(self, index):
        ...
        """
        input_dict, ii_token = self.get_data_info(index)
    ...
    def __getitem__(self, idx):
        ...
        while True:
            data, ii_token = self.prepare_train_data(idx)
            if data is None:
                idx = self._rand_another(idx)
                continue
            return data, ii_token
