...
def lovasz_softmax(probas, labels, classes='present', per_image=False, ignore=None, class_weights=None):
    ...
    """
    if per_image:
        loss = mean(lovasz_softmax_flat(*flatten_probas(prob.unsqueeze(0), lab.unsqueeze(0), ignore), classes=classes, class_weights=class_weights)
                    for prob, lab in zip(probas, labels))
    else:
        with autocast(False):
            loss = lovasz_softmax_flat(*flatten_probas(probas, labels, ignore), classes=classes, class_weights=class_weights)
    return loss
