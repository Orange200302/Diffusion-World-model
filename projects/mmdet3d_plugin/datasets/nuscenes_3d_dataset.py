...
# from mmdet3d.registry import DATASETS as DATASETS_MMDET3D
# from mmdet3d.registry import build_dataset as build_dataset_mmdet3d
# from mmdet.registry import DATASETS, build_dataset
...
class NuScenes3DDataset(Dataset):
    ...
    def __init__(
        ...
        self.ii_world_config = ii_world_dataset
        from mmdet3d.datasets import build_dataset
        if ii_world_dataset is not None:
            # ii_world_dataset['type'] = 'mmdet3d.' + ii_world_dataset['type']
            # self.ii_dataset = build_dataset_mmdet3d(ii_world_dataset)
            self.ii_world_dataset = build_dataset(ii_world_dataset)
        else:
            assert False, "ii_world_dataset is None"
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        data, diff_token = self.get_data_info(idx)
        data["aug_config"] = aug_config
        data = self.pipeline(data)
        
        # ii part
        assert isinstance(idx, int),'idx must be int'
        ii_input_dict, ii_token = self.ii_world_dataset[idx]
        assert ii_token == diff_token,'idx must be equal to token'
        data['ii_input_dict'] = ii_input_dict
    ...
    def get_data_info(self, index):
        ...
        input_dict = dict(
            ...
            map_location=info.get("map_location", "cuda"),
        ...
        return input_dict, info['token']

    def get_ann_info(self, index):# 获取单帧物体标注信息
        info = self.data_infos[index]
        if self.use_valid_flag:
            mask = info["valid_flag"]
        else:
    ...
    def evaluate(
        ...
        if eval_mode['with_planning']:
            ...
            # self.eval_config.update(self.ii_world_config)
            planning_results_dict = planning_eval(results, self.eval_config, logger=logger, ii_config=self.ii_world_config)
