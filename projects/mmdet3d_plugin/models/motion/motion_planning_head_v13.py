...
class V13MotionPlanningHead(BaseModule):
    def __init__(
        ...
        num_map=10,
        # ii part
        ii_transform_head = None,
        ii_cross_attn_layer = None,
    ):
        ...
        self.op_config_map = {
            ...
            "ii_transform": [ii_transform_head, PLUGIN_LAYERS],
            "ii_cross_attn": [ii_cross_attn_layer, ATTENTION]
        }
    ...
    def forward_train(
        ...
        for i, op in enumerate(self.diff_operation_order):
            ...
            elif op == "ii_transform":
                # traj_feature = self.diff_layers[i](
                #     traj_feature,
                #     key=metas["ii_pred"]["pred_latents"],
                #     value=metas["ii_pred"]["pred_latents"],
                # )
                ii_out = self.diff_layers[i](metas["ii_pred"]["pred_latents"])
                # print("waiting to be implemented")
                # import time
                # time.sleep(3)      
                   
            elif op == "ii_cross_attn":
                # 可以把ii合一下
                traj_feature = self.diff_layers[i](
                    traj_feature,
                    ii_out,
